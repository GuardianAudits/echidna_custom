    ...
    case getQuery vmResult of
      ...
      Just q@(PleaseFetchContract addr _ continuation) -> do
        ...
        case config.rpcUrl of
          Just rpcUrl -> do
            ...
            ret <- liftIO $ safeFetchContractFrom session rpcBlock rpcUrl config.fallbackRpcUrls config.rpcTimeout addr
            case ret of
      ...
      Just q@(PleaseFetchSlot addr slot continuation) -> do
        case config.rpcUrl of
          Just rpcUrl -> do
            session <- asks (.fetchSession)
            ret <- liftIO $ safeFetchSlotFrom session rpcBlock rpcUrl config.fallbackRpcUrls config.rpcTimeout addr slot
            case ret of
  ...
  grew' <- liftIO (case lastLoc of
    Just (vec, pc) -> do
      ...
      VMut.read vec pc >>= \case
        (opIx, depths, txResults, hits) | not (txResults `testBit` txResultBit) -> do
          VMut.write vec pc (opIx, depths, txResults `setBit` txResultBit, hits)
        ...
        case maybeCovVec of
          ...
          Just vec -> do
            ...
            when (pc < VMut.length vec) $
              VMut.read vec pc >>= \case
                (_, depths, results, hits) | depth < 64 && not (depths `testBit` depth) -> do
                  let hits' = hits + 1
                  VMut.write vec pc (opIx, depths `setBit` depth, results `setBit` fromEnum Stop, hits')
                  writeIORef covContextRef (True, Just (vec, pc))
                (_, depths, results, hits) -> do
                  let hits' = hits + 1
                  VMut.write vec pc (opIx, depths, results, hits')
